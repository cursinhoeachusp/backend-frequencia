...
router.get("/", async (req, res) => {
  try {
    ...
    const resultadoFinal = alunos.map((aluno) => {
      ...
      const frequencia =
        totalAulas === 0 ? 0 : Math.round((diasGarantidos / totalAulas) * 100);
      ...
      const faltasNaSemana = historico.filter((registro) => {
        ...
        const ehDestaSemana = dayjs(registro.data_aula)
          .tz("America/Sao_Paulo")
          .isSameOrAfter(inicioDaSemana);
